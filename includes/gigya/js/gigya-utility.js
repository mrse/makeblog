...
var gigyaUtil = {
	...
	showLoggedIn: function() {
		...
		if ( gigyaUtil.debug )
			console.log( 'ShowLoggedIn called...' );
	...
	showLoggedOut: function() {
		...
		if ( gigyaUtil.debug )
			console.log( 'ShowLoggedOut called...' );
				...
				if ( undefined !== e ) {
					...
					jQuery.ajax({
						...
						success: function( data, textStatus, jqXHR ) {
							...
							if ( data.complete ) {
								if ( gigyaUtil.debug )
									console.log( 'WP profile info update success!' );
							} else {
								if ( gigyaUtil.debug )
									console.log( 'WP profile info update FAIL!' );
							}
						},
	...
	trackEvent: function( e ) { //for Omniture
		...
		if ( gigyaUtil.debug )
			console.log( 'trackEvent() end reached' );
	},
	testESP: function() {
		var d = {};
		...
		jQuery.ajax({
			...
			success: function( data, textStatus, jqXHR ) {
				...
				if ( data.complete ) {
					if ( gigyaUtil.debug )
						console.log( 'Success! ESP connected!' );
				} else {
					if ( gigyaUtil.debug )
						console.log( 'Fail! ESP not connected :(' );
				}
			},
			error: function( jqXHR, textStatus, errorThrown ) {
				if ( gigyaUtil.debug )
		...
		jQuery.ajax({
			...
			success: function( data, textStatus, jqXHR ) {
				...
				if ( data.complete && null !== data.esp_normalized.acctno ) {
					if ( gigyaUtil.debug )
						console.log( data.esp_normalized );
			...
			complete: function( jqXHR, textStatus ) {
				if ( gigyaUtil.debug )
					console.log( 'searchEspEmail complete: ' + textStatus );
			}
		});
	},
	...
	showEspConfirm: function( esp ) {
		if ( gigyaUtil.debug )
			console.log( 'showEspConfirm() called...' );
...
gigya.accounts.addEventHandlers({
	onLogin: function( e ) {
		...
		// Xheck for ESP account
		if ( gigyaUtil.debug )
			console.log( 'checking if user has esp acct' );

		if ( undefined !== data.is_esp && data.is_esp) { // Make Magazine subscriber
			if ( gigyaUtil.debug )
				console.log( 'esp checkbox checked' );

			if ( undefined === data.esp ) { // User has not linked ESP acct
				if ( gigyaUtil.debug )
					console.log( 'no esp object found' );
			...
			gigya.accounts.getAccountInfo({
				callback: function( r ) {
					if ( r.errorCode === 0 ) {
						...
						if ( gigyaUtil.debug )
							console.log( 'user logged in, no user info, getAccountInfo() failed!' );
					}
				}
			});
		}
